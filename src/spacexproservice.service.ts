...
export class SpacexproserviceService {
  ...
  getAll(launchYear, launchSuccess, landSuccess): Observable<any> {
    return this.Http.get(
      this.Url +"launch_year=" +launchYear +"&launch_success=" +
      launchSuccess + "&land_success=" + landSuccess
    );
  }

  getLaunchLand(launchSuccess, landSuccess): Observable<any> {
    return this.Http.get(
      this.Url + "limit=100" +"&launch_success=" + launchSuccess + "&land_success=" + landSuccess
    );
  }
}
